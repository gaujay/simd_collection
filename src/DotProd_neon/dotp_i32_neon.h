...
#ifndef DOTP_I32_NEON_H
#define DOTP_I32_NEON_H

#include <stdint.h>
#include <arm_neon.h>   // NEON

// SIMD optimization options
#ifndef DOTP32_NEON_SIZE_MULTIPLE
  #define DOTP32_NEON_SIZE_MULTIPLE 0   // 16, 8, 4 (0: no optim)
#endif
//#define DOTP32_NEON_ACCU_2            // Use 2 accumulators (depend on HW/vectors size)


//
static inline int32_t dotProduct_i32_neon_scalar(int32_t const* __restrict u, int32_t const* __restrict v, size_t n)
{
  int32_t res = 0;
...
// 
static inline int32_t dotProduct_i32_neon_naive(int32_t const* __restrict u, int32_t const* __restrict v, size_t n)
{
  int32_t result;
  size_t count = n >> 2;
...
#if DOTP32_NEON_SIZE_MULTIPLE < 4
  n &= 3;
  while (n--)
    result += u[n] * v[n];
#endif
...
//
static inline int32_t dotProduct_i32_neon(int32_t const* __restrict u, int32_t const* __restrict v, size_t n)
{
  int32_t result;
  size_t count = n >> 4;
...
#endif // DOTP32_NEON_SIZE_MULTIPLE < 16

#if DOTP32_NEON_SIZE_MULTIPLE < 8
  // Remaining > 4
  if (n & 4)
  {
    n &= 3;
...
#endif // DOTP32_NEON_SIZE_MULTIPLE < 8
#ifdef DOTP32_NEON_ACCU_2
  // Sum accumulators
...
#endif // DOTP32_NEON_SIZE_MULTIPLE < 4
