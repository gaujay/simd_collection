...
#ifndef DOTP_I8_NEON_H
#define DOTP_I8_NEON_H

#include <stdint.h>
#include <arm_neon.h>   // NEON

// SIMD optimization options
#ifndef DOTP8_NEON_SIZE_MULTIPLE
  #define DOTP8_NEON_SIZE_MULTIPLE 0  // 16, 8 (0: no optim)
#endif
#define DOTP8_NEON_ACCU_2             // Use 2 accumulators (depend on HW/vectors size)

//
static inline int32_t dotProduct_i8_neon_scalar(int8_t const* __restrict u, int8_t const* __restrict v, size_t n)
{
  int32_t res = 0;
...
// 
static inline int32_t dotProduct_i8_neon_naive(int8_t const* __restrict u, int8_t const* __restrict v, size_t n)
{
  int32_t result;
  size_t count = n >> 3;
...
// 
static inline int32_t dotProduct_i8_neon(int8_t const* __restrict u, int8_t const* __restrict v, size_t n)
{
  int32_t result;
  size_t count = n >> 4;
  ...
  // Unroll x2
#if DOTP8_NEON_SIZE_MULTIPLE >= 16
  do
#else
  while (count--)
#endif
  {
    ...
    u0_8 = vmovl_s8( vget_low_s8(u_16) );
    ...
    // 1
    u1_8 = vmovl_s8( vget_high_s8(u_16) );
...
#endif // DOTP8_NEON_SIZE_MULTIPLE < 16
#ifdef DOTP8_NEON_ACCU_2
  // Sum accumulators
...
#endif // DOTP8_NEON_SIZE_MULTIPLE < 8
