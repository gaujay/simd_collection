...
#ifndef DOTP_I16_NEON_H
#define DOTP_I16_NEON_H

#include <stdint.h>
#include <arm_neon.h>   // NEON

// SIMD optimization options
#ifndef DOTP16_NEON_SIZE_MULTIPLE
  #define DOTP16_NEON_SIZE_MULTIPLE 0   // 32, 16, 8 (0: no optim)
#endif
#define DOTP16_NEON_ACCU_2              // Use 2 accumulators (depend on HW/vectors size)


//
static inline int32_t dotProduct_i16_neon_scalar(int16_t const* __restrict u, int16_t const* __restrict v, size_t n)
{
  int32_t res = 0;
...
// 
static inline int32_t dotProduct_i16_neon_naive(int16_t const* __restrict u, int16_t const* __restrict v, size_t n)
{
  int32_t result;
  size_t count = n >> 3;
...
#if DOTP16_NEON_SIZE_MULTIPLE < 8
  n &= 7;
  while (n--)
    result += u[n] * v[n];
#endif
...
//
static inline int32_t dotProduct_i16_neon(int16_t const* __restrict u, int16_t const* __restrict v, size_t n)
{
  int32_t result;
  size_t count = n >> 5;
  ...
  // Unroll x4
#if DOTP168_NEON_SIZE_MULTIPLE >= 32
  do
#else
  while (count--)
#endif
  {
    int16x8_t u0_8, u1_8, u2_8, u3_8;
    int16x8_t v0_8, v1_8, v2_8, v3_8;
    ...
    result0_4 = vmlal_s16(result0_4, vget_low_s16( u2_8), vget_low_s16( v2_8));
    result1_4 = vmlal_s16(result1_4, vget_high_s16(u2_8), vget_high_s16(v2_8));

    // 3
    u3_8 = vld1q_s16(u+24);
    v3_8 = vld1q_s16(v+24);

    result0_4 = vmlal_s16(result0_4, vget_low_s16( u3_8), vget_low_s16( v3_8));
    result1_4 = vmlal_s16(result1_4, vget_high_s16(u3_8), vget_high_s16(v3_8));
...
#if DOTP16_NEON_SIZE_MULTIPLE < 32
  // Unroll remaining x2
  if (n & 16)
  {
    int16x8_t u0_8, u1_8;
...
#endif // DOTP16_NEON_SIZE_MULTIPLE < 32

#if DOTP16_NEON_SIZE_MULTIPLE < 16
  // Remaining > 8
  if (n & 8)
  {
    n &= 7;
...
#endif // DOTP16_NEON_SIZE_MULTIPLE < 16
#ifdef DOTP16_NEON_ACCU_2
  // Sum accumulators
...
#endif // DOTP16_NEON_SIZE_MULTIPLE < 8
